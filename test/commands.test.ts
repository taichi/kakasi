...
test((t: TestContext) => {
    ...
    const aaa = cr.find('#aaa bbb cccc').execute().then((actual: string) => t.is(actual, 'aaa'));
    const ddd = cr.find('#zzz xxxx yyyy').execute().then((actual: string) => t.is(actual, '#zzz xxxx yyyy'));
